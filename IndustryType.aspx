<%@ Page Title="" Language="C#" MasterPageFile="~/FDSS.master" AutoEventWireup="true" CodeFile="IndustryType.aspx.cs" Inherits="IndustryType" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            if (MasterID != null) {
                BindIndustryType(MasterID);
            };

            $("#txtIndustryType").keyup(function () {
                var txtIndustry = $(this).val();
                if (txtIndustry != '') {
                    $("#spanIndustryType").text("");
            ...
            var max = 500;
            var el = document.getElementById('txtIndustryType');

            $('#txtIndustryType').keypress(function (event) {
                var Length = $("#txtIndustryType").val().length;
                var AmountLeft = max - Length;
                $('#txtIndustryType-length-left').html(AmountLeft);
                if (Length >= max) {
                    if (event.which != 8) {
                        $('#spanIndustryType').text('Only Allowed 500 Characters');
        ...
        function BindIndustryType(MasterID) {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "IndustryType.aspx/BindIndustryType",
                ...
                success: function (data) {
                    ...
                    if (data.d != null) {
                        ...
                        $('#txtIndustryType').val(data.d.Description);
        ...
        function SaveReq() {
            var isValid = false;
            if (hasValue("#txtIndustryType", "#spanIndustryType", "Enter the Industry Type"))
                isValid = true;
            if (isValid) {
                var objIndustryType = new IndustryTypes();
                objIndustryType.MasterID = $('#MasterID').val();
                objIndustryType.Description = $('#txtIndustryType').val();
                $("#btnSubmit").attr("disabled", true);

                $.ajax({
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    url: "IndustryType.aspx/SaveorUpdateIndustryType",
                    data: "{objIndustryType : " + ko.toJSON(objIndustryType) + "}",
                    dataType: "json",
                    success: function (data) {
                        if (data.d != "") {
                            ...
                            var inserted = data.d;
                            if (inserted != null) {
                                location.href = "IndustryTypeList.aspx?flagId=" + data.d;
        ...
        function BackToList() {
            location.href = "IndustryTypeList.aspx";
        }
                                ...
                                <div class="card">
                                    <div class="card-block" style="padding: 40px;">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-4">
                                                <input type="text" class="form-control" placeholder="Industry Type" id="txtIndustryType" autocomplete="off" maxlength="500" />
                                                <span class="messages" id="spanIndustryType" style="color: red; font-size: 15px; line-height: 30px;"></span>
                                            </div>
